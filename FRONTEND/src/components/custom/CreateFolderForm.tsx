...
interface NewFolderFormProps {
  article_id: string; // Define the prop type
}

export const NewFolderForm: React.FC<NewFolderFormProps> = ({ article_id }) => {
  const { user } = useAuth()
  ...
  const onSubmit = async (data: { folderName: string; isPublic: boolean, descriptionName:string }) => {
    ...
    try {
     ...
     let created = createSavedArticle({folder_id:result.folder_id, article_id:article_id, user_id:user.id})
      console.log(created)
